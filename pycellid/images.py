...
def box_img(path, im_name, x_pos, y_pos, dx=(15, 15), dy=(15, 15)):
    ...
    # path_n = os.path.join(path + os.sep, im_name)
    # load image
...
def array_img(data, path, channel='BF', n_cells=25, shape=(5,5), cent_cel=[(20, 20),(20, 20)]):
    ...
    :param data: cúal es la data acá?
    :param path: cúal es el path acá?
    :param n_cells: cantidad de cells a representar.
    ...
    :return: La imagen de salida corresponde a ``n_cells``.     
    """
    # Selecciono ucid al azar para las n celulas.
    # Select = np.random.choice(data['ucid'], 91, replace = False).
    # Seleccion de n filas al azar y sin reposición.
    ...
    select['name'] = select.apply(
        lambda row : create_img_name(row['ucid'], row['t_frame'], channel), axis=1
        )
    ...
    select['box_img'] =select.apply(
        lambda row : box_img(path, row['name'], row['xpos'], row['ypos'],[(Y_m, Y_M),(X_m, X_M)]), axis = 1
        )
